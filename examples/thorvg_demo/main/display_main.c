...
{
    int index = MMAP_TEST_JSON;
    while (1) {
        example_create_lottie((index++) % TOTAL_MMAP_FILES);
    ...
    if (NULL == strstr(mmap_assets_get_name(asset_handle, index), ".json")) {
        ...
        while (f < f_total) {
            ...
            if (elaps_frame < (1000 / EXPECTED_FPS)) {
                ...
                ESP_LOGI(TAG,"delayTime:%ld",pdMS_TO_TICKS((1000 / EXPECTED_FPS) - elaps_frame));
            } else {
                vTaskDelay(pdMS_TO_TICKS(1));
                ESP_LOGI(TAG,"delayTime:%ld",pdMS_TO_TICKS(1));
            }
            ESP_LOGI(TAG,"end");
            //vTaskDelay(pdMS_TO_TICKS(1));
